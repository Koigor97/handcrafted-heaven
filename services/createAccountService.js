...
/**
 * Create a user account in the database using OAuth with Google.
 *
 * @param {Object} userData - User account information.
 * @param {string} userData.name - The name of the user.
 * @param {string} userData.email - The email address of the user.
 ...
 * @returns {Promise<Object>} - Returns the newly created user object.
 */
export async function createUserAccountWithGoogle(userData) {
  try {
    const query = `
      INSERT INTO public.users (name, email, oauth_provider, oauth_id, role, address, phone, user_image_url)
